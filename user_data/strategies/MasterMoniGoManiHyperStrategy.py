...
from freqtrade.strategy import BooleanParameter, DecimalParameter, IntParameter, IStrategy, merge_informative_pair, timeframe_to_minutes
...
class MasterMoniGoManiHyperStrategy(IStrategy, ABC):
    ...
    class HyperOpt:
        ...
        def generate_roi_table(params: Dict) -> Dict[int, float]:
            ...
            #logger.info(f'DEBUG --- generate_roi_table -- minimal_roi : {minimal_roi}')

            max_value = max(map(int, minimal_roi.keys()))
            #logger.info(f'DEBUG --- generate_roi_table -- max_value : {max_value}')
            min_value = MasterMoniGoManiHyperStrategy.roi_delay
            #logger.info(f'DEBUG --- generate_roi_table -- min_value : {min_value}')
            f = interp1d(list(map(int, minimal_roi.keys())), list(minimal_roi.values()))
            #logger.info(f'DEBUG --- generate_roi_table -- f : {f}')
            x = list(range(min_value, max_value, step))
            #logger.info(f'DEBUG --- generate_roi_table -- x : {x}')
        ...
        def roi_space() -> List[Dimension]:
            ...
            #logger.info(f'DEBUG --- roi_space -- roi_t_scale: {roi_t_scale}')
            #logger.info(f'DEBUG --- roi_space -- roi_p_scale: {roi_p_scale}')
            ...
            #logger.info(f'DEBUG --- roi_space -- Min ROI / p : {p}')
    ...
    """
    # ---------------------------------------------------
    # Freqtrade protection
    # ---------------------------------------------------
    # Hyperoptable protection parameters
        ...
        for trend in cls.mgm_trends:
            ...
            if cls.mgm_config['trading_during_trends'][f'{space}_trades_when_{trend}'] is True:
                ...
                cls._init_vars(base_cls, space=space, parameter_name=parameter_name,
                               parameter_min_value=signals[name]['min'],
                               parameter_max_value=signals[name]['max'],
                               parameter_threshold=signals[name]['threshold'],
    ...
    def init_util_params(cls, base_cls):
        ...
        for trend in cls.mgm_trends:
            for space in cls.list_of_signal_space:
                for signal_type in cls.list_of_signal_type:
                    if cls.mgm_config['trading_during_trends'][f'{space}_trades_when_{trend}'] is True:
                        ...
                        cls._init_vars(base_cls=base_cls, space=space, parameter_name=param_total_signal_needed,
                                    ...
                                    parameter_max_value=cls.max_total_weight_possible[f'{space}_{signal_type}'],
                                    parameter_threshold=cls.max_total_threshold_possible[f'{space}_{signal_type}'],
                                    precision=cls.precision, overrideable=True)
    ...
    def init_hyperopt_epoch(self) -> None:
        ...
        for trend in self.mgm_trends:
            for space in self.list_of_signal_space:
                for signal_type in self.list_of_signal_type:
